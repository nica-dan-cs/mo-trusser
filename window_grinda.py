...
class window_grinda(window):
	...
	def __init__(self,width,height):
		...
		self.tb_inaltime.place(rely=0.08)
		...
		self.tb_latime.pack()
		self.tb_latime.place(rely=0.15)
		...
		self.label_beton.place(rely=0.33)

		subframe = Frame(self.frame)
		subframe.pack(fill=X)
		subframe.place(rely=0.37)
		...
		self.label_otel.place(rely=0.47)

		subframe = Frame(self.frame)
		subframe.pack(fill=X)
		subframe.place(rely=0.52)
	...
	def run_button_callback(self):
		...
		output = self.process(beton_ales,otel_ales,inaltime,latime,grosime_placa,mom,computations)
	...
	def add_to_file(self,name):
		...
		self.all_written_T_beam["Denumire"].append(str(name).strip("['']"))				
		self.all_written_T_beam["Nr bare + ɸ "].append(ceva[0])
		self.all_written_T_beam["Arie [mm2]"].append(round(ceva[1],2))
		self.all_written_T_beam["Mcap [kNm]"].append(round(ceva[2],2))
		self.all_written_T_beam["ρ [‰]"].append(round(ceva[3]*1000,3))
		
		

	def process(self,beton,otel,inaltime,latime,grosime_placa,mom,computations):
		#clears the labels
		...
		beff=b+2*3*hpl
		M=float(mom)*10**6
		Mpl=hpl*beff*fcd*(d-hpl/2)
		
		#moment taken by shell
		if M<Mpl:
			x=d-sqrt(d**2-((2*M)/(beff*fcd)))
			As=x*beff*fcd/fyd
			Rho_min=max((0.5*beton.fctm)/otel.fyk,1.13/1000)
		...
		#moment too big for the shell
		else:
			messagebox.showinfo("Titlu","Mcalcul>Mplaca putin probabil \nnu este proces facut")
